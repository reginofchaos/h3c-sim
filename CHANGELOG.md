...
- **支持多种真机写法**：`GE1/0/1 to GE1/0/10`（区间）、`g1/0/1,g1/0/5`（离散端口）、`GigabitEthernet 1/0/1 to GigabitEthernet 1/0/10`（全名带空格）
- **range 视图同样支持直切**：端口视图内可直接 `int range ...` 进入批量模式，批量模式下也可直接 `int GE1/0/9` 切回单端口
- **实现方式**：批量在命令引擎层完成——为范围内每个端口临时构造接口视图上下文并重放命令，因此几十条既有接口级命令（`port link-type` / `port access vlan` / `port trunk permit vlan` / `shutdown` / `undo ...`）无需逐个改造即可批量生效，也不会误伤范围外的端口
- **顺带贴近真机**：新增 `undo port link-type`（不带参数，恢复缺省链路类型）；`port trunk permit vlan 10 20` 现在支持真机的空格分隔写法
